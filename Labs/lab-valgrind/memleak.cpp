...
{
  int sum = 0;
  for(int i=0; i < len; i++) //Changed < because it accesses data o/s array
  {
    sum += data[i];
  }
  return sum;
...
char* test2_reverse(char* word)
{
  ...
  otherword[len] = '\0'; //Deleted "+1" because accesses mem outside array
  return otherword; //No need to delete [] word, already deleted in main
}
...
int main(int argc, char* argv[])
{
  ...
  if(test == 1){
    ...
    delete [] data; //Data was not closed
  }
